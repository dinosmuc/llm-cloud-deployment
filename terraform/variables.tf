...
variable "internal_api_key" {
  description = "Internal token shared between nginx and vLLM. Injected by nginx as Authorization: Bearer when proxying upstream, satisfying vLLM's --api-key check. Must differ from public_api_key for defense-in-depth to mean anything."
  type        = string
  sensitive   = true
}
